...
(function (GameUp) {
    ...
    var Client = (function () {
        ...
        Client.prototype.sendRequest = function (callback, url, method, gamerToken, payload) {
            if (!gamerToken) {
                gamerToken = "";
            }
            superagent(method, url).type('application/json').send(payload).timeout(5000).auth(this.apikey, gamerToken).end(function (err, res) {
                if (res.ok) {
                    if (typeof callback.success == 'function') {
        ...
        Client.prototype.listMatches = function (token, callback) {
            this.sendApiRequest(callback, "/gamer/match", "GET", token);
        };
        Client.prototype.getMatch = function (token, matchId, callback) {
            this.sendApiRequest(callback, "/gamer/match/" + matchId, "GET", token);
        };
        Client.prototype.getTurnData = function (token, matchId, fromTurn, callback) {
            this.sendApiRequest(callback, "/gamer/match/" + matchId + "/turn/" + fromTurn, "GET", token);
        };
        Client.prototype.submitTurn = function (token, matchId, lastSeenTurn, nextGamer, data, callback) {
        ...
        Client.prototype.createMatch = function (token, requiredPlayers, callback) {
            var encodedPayload = JSON.stringify({
                "players": requiredPlayers,
            });
            this.sendApiRequest(callback, "/gamer/match/", "POST", token, encodedPayload);
        };
        Client.prototype.leaveMatch = function (token, matchId, callback) {
            var encodedPayload = JSON.stringify({
                "action": "leave",
            });
            this.sendApiRequest(callback, "/gamer/match/" + matchId, "POST", token, encodedPayload);
        };
        Client.prototype.endMatch = function (token, matchId, callback) {
            var encodedPayload = JSON.stringify({
                "action": "end",
            });
            this.sendApiRequest(callback, "/gamer/match/" + matchId, "POST", token, encodedPayload);
